...
class TopModule:
  ...
  def CreateTopModule (Self, OutFilename = None):
    ...
    for Memory in Self.Data['memories']:
      # Expose all Mem Ports
      Output.append ('\t// Exposed %s\n' % (Memory['name']))
      Output.append ('\toutput wire %s_mclk,\n' % (Memory['name']))
      Output.append ('\toutput wire %s_mresetn,\n' % (Memory['name']))
      Output.append ('\toutput wire %s_men,\n' % (Memory['name']))
      Output.append ('\toutput wire [%d:0] %s_maddr,\n' % (Self.Data['bitwidth'] - 1, Memory['name']))
      Output.append ('\toutput wire [%d:0] %s_mdin,\n' % (Self.Data['bitwidth'] - 1, Memory['name']))
      Output.append ('\toutput wire [%d:0] %s_mwe,\n' % ((Self.Data['bitwidth'] / 8) - 1, Memory['name']))
      Output.append ('\tinput wire [%d:0] %s_mdout,\n' % (Self.Data['bitwidth'] - 1, Memory['name']))
      Output.append ('\n')
    ...
    # -- Memories
    for Memory in Self.Data['memories']:
      Output.append ('\tmem_ahb_interpreter #(\n')
      Output.append ('\t\t.DATA_WIDTH (%d),\n' % (Self.Data['bitwidth']))
      Output.append ('\t\t.MEM_BYTES (32\'h%s)\n' % (Memory['size']))
      Output.append ('\t) %s_mem_interpreter_inst (\n' % (Memory['name']))
